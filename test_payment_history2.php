...
// Test with student_id 2482 (NAVANEETH)
...
if (json_last_error() === JSON_ERROR_NONE && isset($json_data['data'][0])) {
                ...
                foreach ($fee_group as $fee) {
                    if (isset($fee['amount_detail']) && !empty($fee['amount_detail']) && $fee['amount_detail'] !== '0') {
                        ...
                        if (is_array($amount_detail) && count($amount_detail) > 0) {
                            echo "Fee: " . ($fee['type'] ?? 'N/A') . " - ₹" . ($fee['amount'] ?? 'N/A') . "\n";
                            ...
                            foreach ($amount_detail as $payment) {
                                ...
                                echo "  ✓ " . ($payment['date'] ?? 'N/A');
                                echo " | ₹" . number_format($amount, 2);
                                echo " | " . ($payment['payment_mode'] ?? 'N/A');
                                if (isset($payment['amount_discount']) && $payment['amount_discount'] > 0) {
                                    echo " | Discount: ₹" . $payment['amount_discount'];
                                }
                                if (isset($payment['amount_fine']) && $payment['amount_fine'] > 0) {
                                    echo " | Fine: ₹" . $payment['amount_fine'];
                                }
                                echo "\n";
                            }
                            echo "\n";
    ...
    echo "Total Paid: ₹" . number_format($total_amount_paid, 2) . "\n";
