...
class Financereports extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->current_session = $this->sch_setting_detail->session_id;
    ...
    public function collection_report()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $fee_session_id = $this->input->post('fee_session_id');

            // Pass selected values back to view for preserving dropdown selections
            $data['selected_class'] = $class_id;
            $data['selected_section'] = $section_id;
            $data['selected_session'] = $session_id;
            $data['selected_fee_session'] = $fee_session_id;
            $data['selected_feetype'] = $feetype_id;
            $data['selected_collect_by'] = $received_by;

            $data['results'] = $this->studentfeemaster_model->getFeeCollectionReport($start_date, $end_date, $feetype_id, $received_by, $group, $class_id, $section_id,$session_id, $fee_session_id);
    ...
    public function combined_collection_report()
    {
        if (!$this->rbac->hasPrivilege('collect_fees', 'can_view')) {
            access_denied();
        }
        ...
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'Reports/finance');
        $this->session->set_userdata('subsub_menu', 'Reports/finance/combined_collection_report');
        $subtotal = false;
        ...
        $data['sch_setting'] = $this->sch_setting_detail;
        $this->load->view('layout/header', $data);
        $this->load->view('financereports/combined_collection_report', $data);
        $this->load->view('layout/footer', $data);
    }
  ...
  $data['start_month'] = $this->sch_setting_detail->start_month;
    ...
    public function getFeeGroupwiseData()
    {
        try {
            ...
            $trend_data = $this->feegroupwise_model->getCollectionTrend(
                $session_id,
                $class_ids,
                $section_ids,
                $feegroup_ids,
    ...
    // ========================================================================
    // FEE COLLECTION DAYSHEET
    // ========================================================================

    /**
     * Display the Fee Collection Daysheet page
     */
    public function daysheet()
    {
        ...
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'Reports/finance');
        $this->session->set_userdata('subsub_menu', 'Reports/finance/daysheet');

        $this->load->model('daysheet_model');
        $this->load->model('addaccount_model');

        $data = array();
        $data['title'] = 'Fee Collection Daysheet';
        $data['sch_setting'] = $this->sch_setting_detail;
        $data['currency_symbol'] = $this->customlib->getSchoolCurrencyFormat();

        // Get accounts for the dropdown filter
        $data['accounts'] = $this->addaccount_model->get();

        // Get active financial year
        $fy = $this->daysheet_model->getActiveFinancialYear();
        $data['financial_year'] = $fy;

        $this->load->view('layout/header', $data);
        $this->load->view('financereports/daysheet_collection', $data);
        $this->load->view('layout/footer', $data);
    }

    /**
     * AJAX endpoint: Get daysheet data
     */
    public function getDaysheetData()
    {
        ...
        try {
            ...
            $fy = $this->daysheet_model->getActiveFinancialYear();
            $fy_start = $fy ? $fy['start_date'] : date('Y-04-01');

            $today       = date('Y-m-d');
            $month_start = date('Y-m-01');

            $response['stats'] = array(
                'today_collection'   => $this->daysheet_model->getTotalCollection($today, $today),
                'month_collection'   => $this->daysheet_model->getTotalCollection($month_start, $today),
                'year_collection'    => $this->daysheet_model->getTotalCollection($fy_start, $today),
                'today_expenses'     => $this->daysheet_model->getTotalExpenses($today, $today),
                'month_expenses'     => $this->daysheet_model->getTotalExpenses($month_start, $today),
                'today_transactions' => $this->daysheet_model->getTransactionCount($today, $today),
                'range_collection'   => $this->daysheet_model->getTotalCollection($from_date, $to_date),
                'range_expenses'     => $this->daysheet_model->getTotalExpenses($from_date, $to_date),
                'range_income'       => $this->daysheet_model->getTotalIncome($from_date, $to_date),
                'range_transactions' => $this->daysheet_model->getTransactionCount($from_date, $to_date)
            );

            $dates           = $this->daysheet_model->getCollectionDates($from_date, $to_date);
            $collection_grid = $this->daysheet_model->getDayWiseCollections($from_date, $to_date);
            $classified      = $this->daysheet_model->getAccountsClassified();

            $all_accounts = array_merge($classified['cash'], $classified['bank']);
            $grid_data    = array();
            foreach ($all_accounts as $acc) {
                ...
                $row = array(
                    ...
                    'is_cash'      => in_array($acc, $classified['cash']),
                    'daily'        => array()
                );
                $row_total = 0;
                foreach ($dates as $d) {
                    $amt = isset($collection_grid[$acc_id][$d]) ? $collection_grid[$acc_id][$d] : 0;
                    $row['daily'][$d] = $amt;
                    $row_total += $amt;
                }
                $row['total'] = $row_total;
            ...
            $response['grid_dates'] = $dates;
            $response['grid_data']  = $grid_data;

            $cash_summaries = array();
            $bank_summaries = array();
            foreach ($classified['cash'] as $acc) {
                $summary = $this->daysheet_model->getAccountSummary($acc['id'], $from_date, $to_date);
                if ($summary['collections'] > 0 || $summary['withdrawals'] > 0 || $summary['transfers_in'] > 0 || $summary['transfers_out'] > 0) {
                    $summary['account_name'] = $acc['name'];
                    $summary['account_id']   = $acc['id'];
                    $cash_summaries[] = $summary;
                }
            }
            foreach ($classified['bank'] as $acc) {
                $summary = $this->daysheet_model->getAccountSummary($acc['id'], $from_date, $to_date);
                if ($summary['collections'] > 0 || $summary['withdrawals'] > 0 || $summary['transfers_in'] > 0 || $summary['transfers_out'] > 0) {
                    $summary['account_name'] = $acc['name'];
                    $summary['account_id']   = $acc['id'];
                    $bank_summaries[] = $summary;
                }
            }
            $response['cash_summaries'] = $cash_summaries;
            $response['bank_summaries'] = $bank_summaries;

            $acct_filter = ($account_id && $account_id != 'all') ? $account_id : null;
            $response['detailed_receipts']    = $this->daysheet_model->getDetailedReceipts($from_date, $to_date, $acct_filter);
            $response['feetype_consolidated'] = $this->daysheet_model->getFeeTypeConsolidated($from_date, $to_date);
            $response['account_consolidated'] = $this->daysheet_model->getAccountConsolidated($from_date, $to_date);
            $response['expenses_by_head']     = $this->daysheet_model->getExpensesByHead($from_date, $to_date);
            $response['income_by_head']       = $this->daysheet_model->getIncomeByHead($from_date, $to_date);
            $response['monthly_trend']        = $this->daysheet_model->getMonthlyTrend($fy_start, $today);
