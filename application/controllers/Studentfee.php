...
class Studentfee extends Admin_Controller
{
    ...
    public function addfee($id)
    {
        ...
        if (isset($hostel_module['is_active']) && $hostel_module['is_active'] && !empty($hostel_room_id)) {
            try {
                // Get all hostel fees for this student (across all sessions)
                $hostel_fees = $this->studenthostelfee_model->getStudentHostelFeesAllSessions($student_session_id, $hostel_room_id);
                ...
                // FIXED: Remove duplicates based on student_hostel_fees.id
                // $unique_hostel_fees = [];
                // $seen_ids = [];
                // foreach ($hostel_fees as $fee) {
                //     if (!in_array($fee->id, $seen_ids)) {
                //         $unique_hostel_fees[] = $fee;
                //         $seen_ids[] = $fee->id;
                //     }
                // }
                // $hostel_fees = $unique_hostel_fees;
        ...
        // Group hostel fees by session
        if (!empty($hostel_fees)) {
            foreach ($hostel_fees as $fee) {
                if (isset($fee->session_id) && !empty($fee->session_id)) {
                    $session_id = $fee->session_id;

                    // Store session information
                    if (!isset($sessions[$session_id])) {
                        // Get session name from session_id
                        $session_info = $this->session_model->get($session_id);
                        if (!empty($session_info) && isset($session_info['session'])) {
                            $sessions[$session_id] = $session_info['session'];
                        } else {
                    ...
                    // Group hostel fees by session
                    if (!isset($hostel_fees_by_session[$session_id])) {
                        $hostel_fees_by_session[$session_id] = array();
                    }

                    $hostel_fees_by_session[$session_id][] = $fee;
                }
            }
        }
        ...
        $data['hostel_fees_by_session'] = $hostel_fees_by_session;
        ...
        // FIXED: Don't pass ungrouped hostel_fees to avoid duplication - only use hostel_fees_by_session
    ...
    public function printMultipleReceipts()
    {
        $record        = $this->input->post('data');
        $record_array  = json_decode($record);
        $setting_result      = $this->setting_model->get();
        ...
        foreach ($record_array as $key => $value) {
            $main_invoice = $value->main_invoice;
            $sub_invoice  = $value->sub_invoice;
            $fee_category = $value->fee_category;
            $student_session_id = $value->student_session_id;
            $fee_session_group_id = isset($value->fee_session_group_id) ? $value->fee_session_group_id : 0;
            $fee_master_id = isset($value->fee_master_id) ? $value->fee_master_id : 0;
            $fee_groups_feetype_id = isset($value->fee_groups_feetype_id) ? $value->fee_groups_feetype_id : 0;

            $fee_record = false;
            $totalfeeList = false;

            if ($fee_category == "transport") {
                $fee_record = $this->studentfeemaster_model->getTransportFeeByInvoice($main_invoice, $sub_invoice);
                if ($fee_record) {
                    $fee_record->fee_category = $fee_category;
                    $fee_record->sub_invoice_id = $sub_invoice;
                    $fee_record->balance = ''; // Transport receipts don't show balance
                }
            } elseif ($fee_category == "hostel") {
                $fee_record = $this->studentfeemaster_model->getHostelFeeByInvoice($main_invoice, $sub_invoice);
                if ($fee_record) {
                    $fee_record->fee_category = $fee_category;
                    $fee_record->sub_invoice_id = $sub_invoice;
                    $fee_record->balance = ''; // Hostel receipts don't show balance
                }
            } elseif ($fee_category == "additional") {
                $fee_record = $this->studentfeemasteradding_model->getFeeByInvoice($main_invoice, $sub_invoice);
                if ($fee_record) {
                    $fee_record->fee_category = $fee_category;
                    $fee_record->sub_invoice_id = $sub_invoice;
                    // Fetch totalfeeList for balance
                    if (!empty($fee_session_group_id) && !empty($fee_master_id) && !empty($fee_groups_feetype_id)) {
                        $totalfeeList = $this->studentfeemasteradding_model->getDueFeeByFeeSessionGroupFeetype($fee_session_group_id, $fee_master_id, $fee_groups_feetype_id, $student_session_id);
                    }
                }
            } else {
                // Regular fees
                $fee_record = $this->studentfeemaster_model->getFeeByInvoice($main_invoice, $sub_invoice);
                if ($fee_record) {
                    $fee_record->fee_category = $fee_category;
                    $fee_record->sub_invoice_id = $sub_invoice;
                    // Fetch totalfeeList for balance calculation (same as printFeesByName)
                    if (!empty($fee_session_group_id) && !empty($fee_master_id) && !empty($fee_groups_feetype_id)) {
                        $totalfeeList = $this->studentfeemaster_model->getDueFeeByFeeSessionGroupFeetype($fee_session_group_id, $fee_master_id, $fee_groups_feetype_id);
                    }
                }
            }

            if ($fee_record) {
                // Calculate balance for regular and additional fees
                if ($totalfeeList) {
                    $fee_discount = 0;
                    $fee_paid = 0;
                    $fee_fine = 0;
                    if (isset($totalfeeList->is_system) && $totalfeeList->is_system) {
                        $totalfeeList->amount = $totalfeeList->student_fees_master_amount;
                    }
                    if (!empty($totalfeeList->amount_detail)) {
                        $fee_deposits = json_decode(($totalfeeList->amount_detail));
                        foreach ($fee_deposits as $fd_key => $fd_value) {
                            $fee_paid = $fee_paid + $fd_value->amount;
                            $fee_discount = $fee_discount + $fd_value->amount_discount;
                            $fee_fine = $fee_fine + $fd_value->amount_fine;
                        }
                    }
                    $fee_record->balance = $totalfeeList->amount - ($fee_paid + $fee_discount);
                } elseif (!isset($fee_record->balance)) {
                    $fee_record->balance = '';
                }

                $receipt_records[] = $fee_record;
            }
        }
        ...
        $data['receipt_records'] = $receipt_records;

        $page = $this->load->view('print/printMultipleReceipts', $data, true);
        echo $page;
    }
    ...
    public function addstudentadditionalfee()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($transport_fees_id != 0 && $fee_category == "transport") {
                ...
                $mailsms_array = $this->feegrouptypeadding_model->getFeeGroupByIDAndStudentSessionID($this->input->post('adding_fee_groups_feetype_id'), $this->input->post('adding_student_session_id'));
