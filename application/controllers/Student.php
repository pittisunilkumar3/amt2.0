...
class Student extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array("timeline_model", "student_edit_field_model", 'transportfee_model', 'marksdivision_model', 'module_model', 'examtype_model', 'publicresultsubjectgroup_model'));
    ...
    //     if (!empty($student_due_feeadding)) {
    //         foreach ($student_due_feeadding as $fee) {
    //             if (isset($fee->session_id) && !empty($fee->session_id) && isset($fee->session) && !empty($fee->session)) {
    //                 $session_id = $fee->session_id;
    ...
    public function view($id)
    {
        ...
        $data['student_due_fee']      = $student_due_fee;
        $siblings = array();
        if (!empty($student['parent_id']) && $student['parent_id'] > 0) {
        ...
        if (!empty($siblings)) {
            foreach ($siblings as $key => $sibling) {
                $siblings[$key]->admi_no = $this->student_model->getAdmissionNumber($sibling->id);
            }
        }
        ...
        // Get admission number from student_admi table
        $admi_no = $this->student_model->getAdmissionNumber($id);
        $student['admi_no'] = $admi_no;
        ...
        // Fetch internal exam results for the student
        $data['internal_results'] = $this->getStudentInternalResults($id);
    ...
    public function create()
    {
        ...
        $this->form_validation->set_rules('reference_id', $this->lang->line('reference_staff'), 'trim|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($insert) {
                ...
                $upload_dir_path  = FCPATH . './uploads/student_documents/' . $insert_id . '/';
    ...
    public function create_doc()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($_FILES["first_doc"]) && !empty($_FILES['first_doc']['name'])) {
                $upload_dir_path = FCPATH . './uploads/student_documents/' . $student_id . '/';
    ...
    public function edit($id)
    {
        ...
        $data['application_doc']       = $this->uploadcontent_model->application_download($id);
        $data['student']               = $student;
        $data["month"]                 = $this->customlib->getMonthDropdown();
    ...
    public function dtstudentlist()
    {
        ...
        if ($search_type == "search_filter") {
            ...
            $resultlist = '{"data":[],"draw":1,"recordsTotal":0,"recordsFiltered":0}';
        }
        ...
        if (!empty($students->data)) {
            foreach ($students->data as $student_key => $student) {
                ...
                $viewbtn = "<a href='" . base_url() . "student/view/" . $student->id . "' class='btn btn-view btn-xs' data-toggle='tooltip' title='" . $this->lang->line('view') . "'><i class='fa fa-eye'></i></a>";

                if ($this->rbac->hasPrivilege('student', 'can_edit')) {
                    $editbtn = "<a href='" . base_url() . "student/edit/" . $student->id . "' class='btn btn-edit btn-xs' data-toggle='tooltip' title='" . $this->lang->line('edit') . "'><i class='fa fa-pencil'></i></a>";
                }

                if ($this->module_lib->hasActive('fees_collection') && $this->rbac->hasPrivilege('collect_fees', 'can_add')) {
                    $collectbtn = "<a href='" . base_url() . "studentfee/addfee/" . $student->student_session_id . "' class='btn btn-collect btn-xs' data-toggle='tooltip' title='" . $this->lang->line('add_fees') . "'><i class='fa fa-dollar'></i></a>";
                }
                ...
                foreach ($fields as $fields_key => $fields_value) {
                    if (!empty(trim($fields_value->name))) {
        ...
        $student_detail_view = $this->load->view('student/_searchDetailView', array('sch_setting' => $sch_setting, 'students' => $students, 'currency_symbol' => $currency_symbol), true);
        $json_data           = array(
    ...
    /**
     * Get all internal exam results for a student organized by session and exam type
     * @param int $student_id Student ID
     * @return array Organized results data
     */
    private function getStudentInternalResults($student_id)
    {
        try {
            // Get all sessions where student has results
            $this->db->distinct();
            $this->db->select('sessions.id as session_id, sessions.session');
            $this->db->from('internalresulttable');
            $this->db->join('sessions', 'sessions.id = internalresulttable.session_id');
            $this->db->where('internalresulttable.stid', $student_id);
            ...
            $results_by_session = array();

            foreach ($sessions as $session) {
                $session_id = $session['session_id'];
                $session_name = $session['session'];

                // Get all exam types for this session
                $this->db->distinct();
                $this->db->select('examtype.id as exam_id, examtype.examtype as exam_name');
                $this->db->from('internalresulttable');
                $this->db->join('examtype', 'examtype.id = internalresulttable.resulgroup_id');
                $this->db->where('internalresulttable.stid', $student_id);
                $this->db->where('internalresulttable.session_id', $session_id);
                $this->db->order_by('examtype.examtype', 'ASC');
                $query = $this->db->get();
                $exam_types = $query->result_array();
                ...
                foreach ($exam_types as $exam_type) {
                    ...
                    // Get all subjects and marks for this exam
                    $this->db->select('
                        internalresulttable.id,
                        internalresulttable.actualmarks,
                        resultsubjects.examtype as subject_name,
                        resultsubjects.subject_code,
                        resultsubject_group_subjects.minmarks,
                        resultsubject_group_subjects.maxmarks
                    ');
                    $this->db->from('internalresulttable');
                    ...
                    $this->db->where('internalresulttable.stid', $student_id);
                    $this->db->where('internalresulttable.resulgroup_id', $exam_id);
                    $this->db->where('internalresulttable.session_id', $session_id);
                    ...
                    $total_min_marks = 0;
                    $pass_status = true;
                    $has_absent = false; // Track if any subject is absent

                    foreach ($subjects as &$subject) {
                        // Check if student is absent (AB)
                        $is_absent = (strtoupper(trim($subject['actualmarks'])) === 'AB');

                        if ($is_absent) {
                            $has_absent = true;
                            $subject['is_absent'] = true;
                            $subject['pass'] = false;
                            $pass_status = false;
                        } else {
                            $subject['is_absent'] = false;
                            $total_marks += (int)$subject['actualmarks'];

                            // Check if student passed this subject
                            $subject['pass'] = ((int)$subject['actualmarks'] >= (int)$subject['minmarks']);
                            if (!$subject['pass']) {
                                $pass_status = false;
                            }
                        }

                        $total_max_marks += (int)$subject['maxmarks'];
                        $total_min_marks += (int)$subject['minmarks'];
                    }

                    // If any subject is absent, mark entire result as absent
                    if ($has_absent) {
                        $pass_status = 'absent';
                    }

                    // Don't calculate percentage if student is absent
                    $percentage = ($has_absent) ? 'AB' : (($total_max_marks > 0) ? round(($total_marks / $total_max_marks) * 100, 2) : 0);

                    $exams_data[] = array(
                        'exam_id' => $exam_id,
                        'exam_name' => $exam_name,
                        'subjects' => $subjects,
                        'total_marks' => $has_absent ? 'AB' : $total_marks,
            ...
            log_message('error', 'Error fetching internal results for student ' . $student_id . ': ' . $e->getMessage());
            return array();
        }
    }
