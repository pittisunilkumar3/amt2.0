...
class Payment extends Studentgateway_Controller
{
        ...
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            ...
            if (!empty($row_counter)) {
                ...
                foreach ($row_counter as $row_key => $row_value) {
                    ...
                    $otherfeecat = $this->input->post("otherfeecat_". $row_value);
                    ...
                    if ($fee_record['fee_category'] == "transport") {
                        ...
                        if($otherfeecat=='otherfee'){
                            $fee_record['student_session_id'] = $this->customlib->getStudentSessionUserID();
                            $result                       = $this->studentfeemasteradding_model->studentDeposit($fee_record);
                        }else{
                            $result                       = $this->studentfeemaster_model->studentDeposit($fee_record);
                        }
                        $fee_record['fee_group_name'] = $result->fee_group_name;
    ...
    public function paymentprocessing()
    {
        ...
        $mailsms_array->student_session_id = $params['student_session_id'];
