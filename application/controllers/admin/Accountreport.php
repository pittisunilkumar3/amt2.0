...
class Accountreport extends Admin_Controller
{
    ...
    {
        $this->session->set_userdata('top_menu', 'Accounting');
        $this->session->set_userdata('sub_menu', 'admin/accountreport');
    ...
    public function search()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $overall_total_credit = 0;
                $overall_total_debit = 0;
                while (strtotime($currentDate) <= strtotime($enddate)) {
                    ...
                    foreach ($dailyTransactions as $trans) {
                        if ($trans['status'] == 'credit') {
                            $overall_total_credit += $trans['amount'];
                        } elseif ($trans['status'] == 'debit') {
                            $overall_total_debit += $trans['amount'];
                        }
                    }
                ...
                $data['total_credit'] = $overall_total_credit;
                $data['total_debit'] = $overall_total_debit;
            }
