...
class Feemaster extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->form_validation->set_rules('fee_groups_id', $this->lang->line('fee_group'), 'required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            foreach ($feetype_ids as $feetype_id) {
                ...
                if ($this->feesessiongroup_model->check_data_exists($fee_groups_id, $feetype_id, 0)) {
                    continue; // Skip if already exists
                }

                $amount = $this->input->post('amount_' . $feetype_id);
                $due_date = $this->input->post('due_date_' . $feetype_id);
                $fine_type = $this->input->post('account_type_' . $feetype_id);
                $fine_percentage = $this->input->post('fine_percentage_' . $feetype_id);
                $fine_amount = $this->input->post('fine_amount_' . $feetype_id);

                if($fine_amount){
                    $fine_amount_val = convertCurrencyFormatToBaseAmount($fine_amount);
                }else{
                    $fine_amount_val = 0;
                }
                ...
                $insert_array = array(
                    ...
                    'fine_type'       => $fine_type,
                    'fine_percentage' => $fine_percentage ? $fine_percentage : 0,
