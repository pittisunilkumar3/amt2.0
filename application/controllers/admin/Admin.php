...
class Admin extends Admin_Controller
{
    ...
    public function searchhole()
    {
        $data['title']           = 'Search';
        $search_text             = $this->input->post('search_text2');
        $data['sch_setting']     = $this->sch_setting_detail;
        $data['search_text']     = trim($this->input->post('search_text2'));
        $userdata                = $this->customlib->getUserData();
        $data['adm_auto_insert'] = $this->sch_setting_detail->adm_auto_insert;
        $carray                  = array();
        $class                   = $this->class_model->get();
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/searchhole', $data);
        $this->load->view('layout/footer', $data);
    }

    public function search_texthole()
    {
        $search_text1 = $this->input->post('search_text');
        $params       = array('search_text' => $search_text1);
        $array        = array('status' => 1, 'error' => '', 'params' => $params);
    ...
    public function dtstudentlisthole()
    {
        $search_text     = $this->input->post('search_text');
        $sch_setting     = $this->sch_setting_detail;
        $currency_symbol = $this->customlib->getSchoolCurrencyFormat();
        ...
        $resultlist      = $this->student_model->searchFullTexthole($search_text, $carray);
        $start           = $this->input->post('start');
        $length          = $this->input->post('length');
