...
class Cron extends MY_Controller
{
    ...
    /**
     * Auto Generate Payroll via Cron
     * URL: /cron/autoGeneratePayroll/{cron_secret_key}
     * Should run daily at midnight — only generates on configured day
     */
    public function autoGeneratePayroll($key = "")
    {
        error_reporting(E_ALL);
        ini_set('display_errors', 1);
        ob_implicit_flush(true);

        if (!$this->cron_key || $this->cron_key != $key) {
            echo "Invalid key\n";
            return;
        }
        ...
        try {
            ...
            if (empty($settings) || $settings['auto_payroll_enabled'] !== 'yes') {
                echo "Auto payroll is disabled\n";
                return;
            }

            $targetDay = (int) $settings['auto_payroll_day'];
            $currentDay = (int) date('j');

            if ($currentDay !== $targetDay) {
                echo "Today is day $currentDay, auto payroll runs on day $targetDay\n";
                return;
            }
            ...
            echo "Auto-generating payroll for $prevMonth $prevYear...\n";
            ob_flush();

            $result = $this->payroll_model->autoGeneratePayroll($prevMonth, $prevYear, 0);

            echo "Generated: " . $result['generated'] . "\n";
