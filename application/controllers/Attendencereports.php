...
class Attendencereports extends Admin_Controller
{
    ...
    public function staffattendancereport()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $date_result        = array();
            $monthAttendance    = array();
            $all_staff_ids      = array(); // Track all unique staff members
            ...
            for ($i = 1; $i <= $num_of_days; $i++) {
                ...
                foreach ($res as $result_k => $result_v) {
                    ...
                    // Get all attendance records for this staff on this date to calculate hours
                    $hours_worked = '-';
                    $hours_decimal = 0;
                    
                    try {
                        // Get detailed attendance records with check-in/check-out times
                        if (method_exists($this->staffattendancemodel, 'getStaffDayAttendance')) {
                            $staff_day_records = $this->staffattendancemodel->getStaffDayAttendance($result_v['id'], $att_date);
                            
                            if (!empty($staff_day_records) && is_array($staff_day_records)) {
                                $first_checkin_time = null;
                                $last_checkout_time = null;
                                
                                // Process all records to find first check-in and last check-out
                                foreach ($staff_day_records as $record) {
                                    // Check for check-in time
                                    if (!empty($record['check_in_time'])) {
                                        $checkin_timestamp = strtotime($record['check_in_time']);
                                        if ($checkin_timestamp !== false) {
                                            if ($first_checkin_time === null || $checkin_timestamp < $first_checkin_time) {
                                                $first_checkin_time = $checkin_timestamp;
                                            }
                                        }
                                    }
                                    ...
                                    if (!empty($record['check_out_time'])) {
                                        $checkout_timestamp = strtotime($record['check_out_time']);
                                        if ($checkout_timestamp !== false) {
                                            if ($last_checkout_time === null || $checkout_timestamp > $last_checkout_time) {
                                                $last_checkout_time = $checkout_timestamp;
                                            }
                                        }
                                    }
                                }
                                ...
                                if ($first_checkin_time !== null && $last_checkout_time !== null && $last_checkout_time > $first_checkin_time) {
                                    $duration_seconds = $last_checkout_time - $first_checkin_time;
                                    
                                    if ($duration_seconds > 0) {
                                        $hours = floor($duration_seconds / 3600);
                                        $minutes = floor(($duration_seconds % 3600) / 60);
                                        $hours_worked = sprintf('%d:%02d', $hours, $minutes);
                                        $hours_decimal = round($duration_seconds / 3600, 2);
                                    }
                                }
                                
                                // Store the times for display in popover
                                $result_v['first_checkin_time'] = $first_checkin_time ? date('h:i A', $first_checkin_time) : '';
                                $result_v['last_checkout_time'] = $last_checkout_time ? date('h:i A', $last_checkout_time) : '';
                            }
                        }
                    } catch (Exception $e) {
                        // Silently handle any errors and continue without hours calculation
                        log_message('error', 'Error calculating staff hours: ' . $e->getMessage());
                    }
                    
                    $result_v['hours_worked'] = $hours_worked;
                    $result_v['hours_decimal'] = $hours_decimal;
                    
                    $s[$result_v['id']] = $result_v;
                    
                    // Track all unique staff members across all dates
                    if (!isset($all_staff_ids[$result_v['id']])) {
                        $all_staff_ids[$result_v['id']] = $result_v;
                    }
                }
            ...
            // Calculate month attendance and total hours for ALL staff members found in any day
            $monthlyHours = array();
            foreach ($all_staff_ids as $staff_id => $staff_info) {
                $date              = $searchyear . "-" . $month;
                $newdate           = date('Y-m-d', strtotime($date));
                ...
                foreach ($date_result as $day_date => $day_staff) {
                    if (isset($day_staff[$staff_id]) && isset($day_staff[$staff_id]['hours_decimal'])) {
                        $total_hours_decimal += $day_staff[$staff_id]['hours_decimal'];
                    }
                }
                
                $total_hours = floor($total_hours_decimal);
                $total_minutes = round(($total_hours_decimal - $total_hours) * 60);
                $monthlyHours[$staff_id] = sprintf('%d:%02d hrs', $total_hours, $total_minutes);
            }
            
            // Use all_staff_ids as the complete staff list for the report
            $student_result = array_values($all_staff_ids);
