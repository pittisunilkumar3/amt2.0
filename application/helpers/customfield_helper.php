...
if (!function_exists('display_custom_fields')) {
    ...
    function display_custom_fields($belongs_to, $rel_id = false, $where = array())
    {
        ...
        foreach ($result as $result_key => $field) {
            ...
            if ($field['type'] == 'input' || $field['type'] == 'number') {
                ...
                $fields_html .= render_input_field($field_name, $field['belong_to'], $field['id'], $field['validation'], $label, $value, $type, $input_class,$field['length']);
    ...
    function render_input_field($name, $belong_to, $field_id, $validation, $label = '', $value = '', $type = 'text', $input_class = '',$length)
    {
        $input            = '';
        $_form_group_attr = '';
        $_input_attrs     = '';
        ...
        $input .= '<input type="' . $type . '" id="' . $name . '"  maxlength="' . $length . '"  name="' . $name . '" class="form-control' . $input_class . '" ' . $_input_attrs . ' value="' . $value . '">';

        $input .= '<span class="text-danger">' . form_error($name) . '</span>';
        $input .= '</div>';

        if ($type === 'number' && is_numeric($length)) {
            $input .= '<script>
                document.getElementById("' . $name . '").addEventListener("input", function() {
                    var inputValue = this.value.toString();
                    var maxDigits = ' . $length . ';
