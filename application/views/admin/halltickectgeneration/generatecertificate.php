        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        <div class="row">
                            <form role="form" action="<?php echo site_url('admin/halltickectgeneration/search') ?>" method="post" class="">
                                ...
                                <div class="col-sm-3">
                                    <div class="form-group">
                                        ...
                                        <select id="subjectgrp_id" name="subjectgrp_id" class="form-control" >
                                            <option value=""><?php echo $this->lang->line('select'); ?></option>
                                            <?php
                                                if (isset($getsubjectgroups)) {
                                                    foreach ($getsubjectgroups as $list) {
                                                            ?>
                                                        <option value="<?php echo $list['sgid']; ?>" <?php if (set_value('subjectgrp_id') == $list['sgid']) {
                                                            echo "selected=selected";
                                                        }
                                                        ?>><?php echo $list['subjectgrp_name']; ?>
                                                        </option>
                                                        <?php
                                                    }
                                                }
                                        ...
                                        <span class="text-danger"><?php echo form_error('subjectgrp_id'); ?></span>
                                    </div>
                                </div>
                                        ...
                                        <select name="certificate_id" class="form-control" >
                                            ...
                                            <?php
                                                if (isset($certificateList)) {
                                                    ...
                                                    ?>><?php echo $list->halltickect_name; ?></option>
                                                    <?php
                                        }
                                        }
                                        ?>
                    ...
                    if (isset($resultlist)) {
                        ?>
                        <form method="post" action="<?php echo base_url('admin/halltickectgeneration/generatemultiple') ?>">
                            <div  class="" id="duefee">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#select_all').on('click', function () {
            ...
            // Use DataTables API to access all rows (including those on other pages)
        ...
        $(document).on('click', '.checkbox', function () {
            ...
            // Get all checkboxes across all pages using DataTables API
            table.rows().every(function() {
                var row = this.node();
            ...
            var allChecked = allCheckboxes.length > 0 && allCheckboxes.every(function(cb) {
                return cb.checked;
            });
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('click', '.printSelected', function () {
            ...
            var subjectgrpId = $("#subjectgrp_id").val();

            console.log(subjectgrpId);

            // Use DataTables API to access all checked checkboxes across all pages
            table.rows().every(function() {
                var row = this.node();
                var checkbox = $(row).find('input[name="check"]:checked');
                
                if (checkbox.length > 0) {
                    var studentId = checkbox.data('student_id');
                    item = {}
            ...
            if (array_to_print.length == 0) {
                ...
                $.ajax({
                    url: '<?php echo site_url("admin/halltickectgeneration/generatemultiple") ?>',
                    type: 'post',
                    dataType: "html",
                    data: {'data': JSON.stringify(array_to_print), 'subjectgrp_id':subjectgrpId,'class_id': classId, 'certificate_id': certificateId,'section_id':sectionId, },
