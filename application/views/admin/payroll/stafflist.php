...
?>  value="<?php echo date("Y") ?>"><?php echo date("Y") ?></option>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <a href="<?php echo base_url() ?>admin/payroll/payrollSettings" class="btn btn-default btn-sm" data-toggle="tooltip" title="<?php echo $this->lang->line('payroll_auto_settings'); ?>">
                                            <i class="fa fa-cogs"></i> <?php echo $this->lang->line('payroll_settings'); ?>
                                        </a>
    ...
    ?>
                        <div class="box-header ptbnull">
                            ...
                            <div class="box-tools pull-right">
                                ...
                                <button type="button" onclick="autoGenerateAll()" class="btn btn-warning btn-sm" data-toggle="tooltip" title="<?php echo $this->lang->line('auto_generate_all'); ?>">
                                    <i class="fa fa-magic"></i> <?php echo $this->lang->line('auto_generate_all'); ?>
                                </button>
                                <?php } ?>
                            </div>
                        </div>
...
?>';
    ...
    // Auto Generate All Payroll - defined OUTSIDE $(document).ready() so onclick can access it
    function autoGenerateAll() {
        var monthVal = $('select[name="month"]').val();
        var yearVal = $('select[name="year"]').val();

        if (!monthVal || monthVal === 'select' || !yearVal || yearVal === 'select') {
            alert('<?php echo $this->lang->line("select_month_year_first"); ?>');
            return;
        }

        var monthName = $('select[name="month"] option:selected').text();

        if (!confirm('<?php echo $this->lang->line("auto_generate_confirm"); ?>\n\n' + monthName + ' ' + yearVal)) {
            return;
        }

        var btn = $('button[onclick="autoGenerateAll()"]');
        btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> <?php echo $this->lang->line("generating"); ?>...');

        $.ajax({
            url: '<?php echo site_url("admin/payroll/autoGenerate"); ?>',
            type: 'POST',
            data: {
                month: monthVal,
                year: yearVal,
            ...
            success: function(response) {
                btn.prop('disabled', false).html('<i class="fa fa-magic"></i> <?php echo $this->lang->line("auto_generate_all"); ?>');
                if (response.status === 'success') {
                    alert(response.message);
                    window.location.reload();
                } else {
                    alert(response.message || '<?php echo $this->lang->line("error_message"); ?>');
                }
            },
            error: function() {
                btn.prop('disabled', false).html('<i class="fa fa-magic"></i> <?php echo $this->lang->line("auto_generate_all"); ?>');
                alert('<?php echo $this->lang->line("error_message"); ?>');
            }
        });
    }
</script>