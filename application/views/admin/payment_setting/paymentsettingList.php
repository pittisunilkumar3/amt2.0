...
#payment-settings-page .nav-tabs { display: none !important; }
#payment-settings-page .box-header { width: 100%; text-align: center; border-bottom: none; margin-bottom: 20px; padding: 0; }
#payment-settings-page .box-title { font-size: 1.5rem; font-weight: 700; color: #333; margin-bottom: 10px;}

/* Content Area (Left) */
#payment-settings-page .tab-content {
    width: 75%;
    padding-right: 40px;
...
.gateway-item:hover { color: #000; }
.gateway-item input { margin-right: 12px; margin-left: 2px; cursor: pointer; accent-color: #f97316; transform: scale(1.2); }
.gateway-item.active { font-weight: 600; color: #000; }

/* Form Elements */
#payment-settings-page .form-group { margin-bottom: 25px; }
#payment-settings-page .form-group label {
    font-weight: 600;
    color: #475569;
    padding-top: 10px;
    font-size: 1.3rem;
...
#payment-settings-page .form-control {
...
#payment-settings-page .form-control:focus {
    background-color: #e0f2fe;
    box-shadow: 0 0 0 2px #bae6fd;
}

/* Save Button */
#payment-settings-page .btn-primary {
...
#payment-settings-page .btn-primary:hover {
    transform: translateY(-2px);
    background: #1d4ed8 !important;
}
#payment-settings-page .box-footer {
...
.gateway-list::-webkit-scrollbar { width: 5px; }
.gateway-list::-webkit-scrollbar-track { background: #f1f1f1; border-radius: 10px; }
.gateway-list::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 10px; }
.gateway-list::-webkit-scrollbar-thumb:hover { background: #94a3b8; }


/* Promo Box (Logo) - Move to Left */
#payment-settings-page .box-body .row {
    display: flex;
    align-items: center;
    flex-wrap: wrap;
}
/* Logo Container */
#payment-settings-page .box-body .row .col-md-5 {
    order: 1; /* First */
    width: 35%;
    text-align: center;
    padding-right: 30px;
...
#payment-settings-page .box-body .row .col-md-7 {
    order: 2; /* Second */
    width: 65%;
    padding-left: 20px;
...
#payment-settings-page .disblock {
    text-align: center;
}
#payment-settings-page .disblock h5 {
    color: #0ea5e9;
    font-weight: 600;
    margin-bottom: 20px;
    font-size: 1.1rem;
}
#payment-settings-page .disblock img {
    border-radius: 12px;
...
#payment-settings-page .disblock img:hover {
    transform: scale(1.02);
}

/* Responsive */
@media (max-width: 992px) {
    #payment-settings-page .tab-content { width: 100%; border-right: none; margin-bottom: 30px; order: 2; }
    .payment-sidebar-col { width: 100%; padding-left: 0; margin-bottom: 30px; order: 1; display:flex; flex-wrap: wrap;}
    .payment-sidebar-col h4 { width: 100%; }
    .gateway-item { width: 50%; }
}
</style>

<script>
document.addEventListener('DOMContentLoaded', function() {
    // Check if sidebar already exists (to prevent dupes on aggressive reloads/renders)
    if(document.querySelector('.payment-sidebar-col')) return;

    var container = document.getElementById('payment-settings-page');
    if(!container) return; // Grid view or error

    // Generate Layout
    var sidebarHtml = '<div class="payment-sidebar-col"><h4>Select Payment Gateway</h4><div class="gateway-list">';
    
    // Get Tabs
    var tabs = container.querySelectorAll('.nav-tabs li');
    tabs.forEach(function(tab) {
        var link = tab.querySelector('a');
        var tabId = link.getAttribute('href');
        var tabName = link.textContent;
        var isActive = tab.classList.contains('active');
        
        sidebarHtml += '<label class="gateway-item ' + (isActive ? 'active' : '') + '" data-target="' + tabId + '">';
        sidebarHtml += '<input type="radio" name="payment_method_sel" ' + (isActive ? 'checked' : '') + '>';
        sidebarHtml += '<span>' + tabName + '</span>';
        sidebarHtml += '</label>';
    });
    ...
    var radios = container.querySelectorAll('.gateway-item');
    radios.forEach(function(radioItem) {
        radioItem.addEventListener('click', function(e) {
            // e.preventDefault(); // Don't prevent default, let radio check
            var targetId = this.getAttribute('data-target');
            
            // Trigger Bootstrap Tab
            // Since we don't have direct access to 'click' on hidden element cleanly without JQuery, use JQuery as it's loaded
...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="row">
            <div class="col-md-10">
                <div id="payment-settings-page" class="nav-tabs-custom box box-primary theme-shadow">
