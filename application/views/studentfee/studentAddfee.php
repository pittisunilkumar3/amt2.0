<style type="text/css">
    ...
    /* Modern Student Profile Card - CSS Grid Layout */
    ...
    /* Info Boxes (Advance Payment Section) */
    .info-box {
        border-radius: var(--card-radius);
    ...
    /* Specific Colors for Info Boxes to override AdminLTE defaults if needed, 
       but keeping the classes bg-green, bg-blue etc. works well if we just clean up the container */
    .bg-green {
    ...
    /* ===== Fee Row Status Background Colors ===== */
    /* Using maximum specificity selectors to override ALL external CSS */
    table.table-striped.table-bordered.table-hover tbody tr.fee-status-paid td,
    table.example tbody tr.fee-status-paid td,
    .table-responsive table tbody tr.fee-status-paid td {
        background-color: #d4edda !important;
        color: #155724 !important;
    }
    /* Partial Payment - Yellow */
    table.table-striped.table-bordered.table-hover tbody tr.fee-status-partial td,
    table.example tbody tr.fee-status-partial td,
    .table-responsive table tbody tr.fee-status-partial td {
        background-color: #fff3cd !important;
        color: #856404 !important;
    }
    /* Partial Payment + Overdue - Orange */
    table.table-striped.table-bordered.table-hover tbody tr.fee-status-partial-overdue td,
    table.example tbody tr.fee-status-partial-overdue td,
    .table-responsive table tbody tr.fee-status-partial-overdue td {
        background-color: #ffe0b2 !important;
        color: #e65100 !important;
    }
    /* Unpaid (not overdue) - Light Red */
    table.table-striped.table-bordered.table-hover tbody tr.fee-status-unpaid td,
    table.example tbody tr.fee-status-unpaid td,
    .table-responsive table tbody tr.fee-status-unpaid td {
        background-color: #fce4ec !important;
        border-top: 1px solid #f8bbd0 !important;
    }
    /* Overdue Unpaid - Red (danger class) */
    table.table-striped.table-bordered.table-hover tbody tr.danger td,
    table.example tbody tr.danger td,
    .table-responsive table tbody tr.danger td {
        background-color: #f2dede !important;
        color: #a94442 !important;
    }
    /* Deposit detail sub-rows */
    table.table-striped.table-bordered.table-hover tbody tr.white-td td,
    table.example tbody tr.white-td td,
    .table-responsive table tbody tr.white-td td {
        background-color: #ffffff !important;
    }
    /* Session header row */
    table.table-striped.table-bordered.table-hover tbody tr.session-header td,
    table.example tbody tr.session-header td {
        background: #fff !important;
    }
    ...
    /* Fixed sidebar toggle button - Anchored to right edge */
    .studentsideopen {
        cursor: pointer;
        position: fixed;
        top: 84px;
    ...
    /* Sidebar Header */
    .studentsidetopfixed {
        background: linear-gradient(135deg, #3c8dbc 0%, #2d7baa 100%);
    ...
    .studenttaps::-webkit-scrollbar {
        height: 4px;
    }

    .studenttaps::-webkit-scrollbar-track {
        background: rgba(255, 255, 255, 0.1);
        border-radius: 2px;
    }

    .studenttaps::-webkit-scrollbar-thumb {
    ...
    .studentsidebar .tab-content::-webkit-scrollbar {
        width: 8px;
    }

    .studentsidebar .tab-content::-webkit-scrollbar-track {
        background: #f1f1f1;
    }

    .studentsidebar .tab-content::-webkit-scrollbar-thumb {
        background: #ccc;
        border-radius: 4px;
    ...
    .studentsidebar .tab-content::-webkit-scrollbar-thumb:hover {
        background: #bbb;
    }
    ...
    /* Active indicator strip (right side) to allow content to align left */
    .studentname {
    ...
    .studentname:hover .student-tittle {
        color: #3c8dbc;
    }

    /* --- Modern Isolated Sidebar Styles --- */

    /* Variables for this component only */
    :root {
        --sb-width: 350px;
        --sb-bg: #ffffff;
        --sb-header-bg: #3c8dbc;
    ...
    .modern-sidebar-container.active {
        right: 0;
    }
    ...
    /* Scrollbar for tabs */
    .modern-sidebar-tabs::-webkit-scrollbar {
        height: 4px;
    }

    .modern-sidebar-tabs::-webkit-scrollbar-track {
        background: rgba(255, 255, 255, 0.1);
    }

    .modern-sidebar-tabs::-webkit-scrollbar-thumb {
        background: rgba(255, 255, 255, 0.5);
        border-radius: 2px;
    }

    .modern-sidebar-tabs li {
        flex: 0 0 auto;
        margin: 0 !important;
        padding: 0 !important;
    ...
    .modern-sidebar-tabs li.active a,
    .modern-sidebar-tabs li a:hover {
        background: #fff;
    ...
    .modern-sidebar-content {
        ...
        /* Fallback height if flex fails */
        background: #fdfdfd;
        position: relative;
        padding-bottom: 50px;
        /* Space for bottom scroll access */
    }

    /* Ensure tab pane doesn't block scroll */
    .tab-content>.tab-pane {
        height: 100%;
        /* Pass height down */
        overflow-y: auto !important;
        /* Allow pane to scroll if needed */
        padding-bottom: 20px;
    }
    ...
    .modern-student-avatar-container {
        flex: 0 0 50px;
        height: 50px;
        margin-right: 15px;
        display: flex;
    ...
    .modern-student-item:hover .modern-student-name {
...
<div class="content-wrapper">
    <div class="row">
        ...
        <div>
            <a id="modernSidebarTrigger" class="modern-sidebar-toggle"><i class="fa fa-bars"></i></a>

            <!-- Sidebar Overlay -->
            <div class="modern-sidebar-overlay"></div>

            <aside class="modern-sidebar-container">
                <div class="stutop" id="">
                    <!-- Create the tabs -->
                    <!-- Modern Header with Grid Layout -->
                    <div class="modern-sidebar-header">
                        <div class="modern-sidebar-title-row">
                            <h4 class="modern-sidebar-class-title"><?php echo $student["class"]; ?></h4>
                            <div class="modern-sidebar-close studentsideclose"><i class="fa fa-times"></i></div>
                        </div>

                        <!-- Scrollable Tabs -->
                        <ul class="modern-sidebar-tabs">
                            <?php foreach ($class_section as $skey => $svalue) { ?>
                                <li class="<?php echo ($student["section_id"] == $svalue["section_id"]) ? 'active' : ''; ?>">
                    ...
                    <!-- Tab panes -->
                    <div class="modern-sidebar-content tab-content">
                        ...
                        ?>
                            <div class="tab-pane <?php
                                ...
                                ?>
                                        <a href="<?php echo base_url() . "studentfee/addfee/" . $stvalue["student_session_id"] ?>"
                                            class="modern-student-item">
                                            <div class="modern-student-avatar-container">
                                                <img src="<?php echo base_url() . $stvalue["image"] . img_time(); ?>"
                                                    alt="User" class="modern-student-avatar">
                                            </div>
                                            <div class="modern-student-info">
                                                <div class="modern-student-name">
            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header">
                        ...
                        <div class="box-tools">
                            <a href="<?php echo base_url() ?>studentfee" class="btn btn-primary btn-xs">
                                ...
                                <div class="student-profile-card">
                                    ...
                                    <div class="student-profile-details">
                                        <div class="profile-item">
                                            <span class="profile-label"><?php echo $this->lang->line('name'); ?></span>
                                            <span class="profile-value"><?php echo $this->customlib->getFullName($student['firstname'], $student['middlename'], $student['lastname'], $sch_setting->middlename, $sch_setting->lastname); ?></span>
                                        </div>
                                        <div class="profile-item">
                                            <span class="profile-label"><?php echo $this->lang->line('class_section'); ?></span>
                                            <span class="profile-value"><?php echo $student['class'] . " (" . $student['section'] . ")" ?></span>
                                        </div>
                                        <div class="profile-item">
                                            <span class="profile-label"><?php echo $this->lang->line('father_name'); ?></span>
                                            <span class="profile-value"><?php echo $student['father_name']; ?></span>
                                        </div>
                                        <div class="profile-item">
                                            <span class="profile-label"><?php echo $this->lang->line('admission_no'); ?></span>
                                            <span class="profile-value"><?php echo $student['admission_no']; ?></span>
                                        </div>
                                        <div class="profile-item">
                                            <span class="profile-label"><?php echo $this->lang->line('mobile_number'); ?></span>
                                            <span class="profile-value"><?php echo $student['mobileno']; ?></span>
                                        </div>
                                        <div class="profile-item">
                                            <span class="profile-label"><?php echo $this->lang->line('roll_number'); ?></span>
                                            <span class="profile-value"><?php echo $student['roll_no']; ?></span>
                                        </div>
                                        <div class="profile-item">
                                            <span class="profile-label"><?php echo $this->lang->line('category'); ?></span>
                                        ...
                                        <?php if ($sch_setting->rte) { ?>
                                            <div class="profile-item">
                                                <span class="profile-label"><?php echo $this->lang->line('rte'); ?></span>
                                                <span class="profile-value text-danger"><?php echo $student['rte']; ?></span>
                                            </div>
                                        <?php } ?>
                                    </div>
                                </div>
                        ...
                        <div class="row no-print mb10">
                            <div class="col-md-12 mDMb10">
                                <div class="float-rtl-right float-left">
                                    ...
                                    <button type="button" class="btn btn-sm btn-success printMultipleReceipts" id="load"
                                        data-loading-text="<i class='fa fa-spinner fa-spin '></i> <?php echo $this->lang->line('please_wait') ?>"><i
                                            class="fa fa-print"></i>
                                    ...
                                    ?>
                                            ...
                                            ?>
                                                ...
                                                <?php
                                                        ...
                                                        } else if (($fee_value->due_date != "0000-00-00" && $fee_value->due_date != null) && (strtotime($fee_value->due_date) < strtotime(date('Y-m-d')))) {
                                            ?>
                                                ...
                                                ?>
                                                        <tr class="white-td">
                                                            <td align="left">
                                                                <input class="receipt_checkbox" type="checkbox" name="receipt_checkbox"
                                ...
                                <tbody class="session-fees-container" data-session-id="hostel-<?php echo $session_id; ?>">
                                    ...
                                    ?>
                                            ...
                                            <?php
                                                                ...
                                                                } else if (($hostel_fee_value->due_date != "0000-00-00" && $hostel_fee_value->due_date != null) && (strtotime($hostel_fee_value->due_date) < strtotime(date('Y-m-d')))) {
                                    ?>
                                            <tr class="danger font12" style="background-color: #f2dede !important;">
                                            <?php
                                                                } else {
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // Print Multiple Receipts - select sub-payment checkboxes to generate combined receipt
        $(document).on('click', '.printMultipleReceipts', function() {
            var array_to_print = [];
            var $this = $(this);
            $.each($("input[name='receipt_checkbox']:checked"), function() {
                var main_invoice = $(this).data('main_invoice');
                var sub_invoice = $(this).data('sub_invoice');
                var fee_category = $(this).data('fee_category');
                var student_session_id = $(this).data('student_session_id');
                var fee_session_group_id = $(this).data('fee_session_group_id') || 0;
                var fee_master_id = $(this).data('fee_master_id') || 0;
                var fee_groups_feetype_id = $(this).data('fee_groups_feetype_id') || 0;

                var item = {};
                item["main_invoice"] = main_invoice;
                item["sub_invoice"] = sub_invoice;
                item["fee_category"] = fee_category;
                item["student_session_id"] = student_session_id;
                item["fee_session_group_id"] = fee_session_group_id;
                item["fee_master_id"] = fee_master_id;
            ...
            if (array_to_print.length === 0) {
                alert("Please select at least one payment receipt checkbox to print.");
            } else {
                $.ajax({
                    url: '<?php echo site_url("studentfee/printMultipleReceipts") ?>',
                    type: 'post',
                    data: {
                        'data': JSON.stringify(array_to_print)
                    },
        ...
        // Select All for receipt checkboxes
        $(document).on('click', '#select_all_receipt', function() {
            if ($(this).is(':checked')) {
                $('.receipt_checkbox').prop('checked', true);
            } else {
...
<script type="text/javascript">
    ...
    $('#adding-confirm-delete').on('click', '.btn-ok', function(e) {
        ...
        $.ajax({
            ...
            dataType: 'JSON',
            data: {
                'main_invoice': adding_main_invoice,
                'sub_invoice': adding_sub_invoice
            },
            success: function(data) {
                $modalDiv.modal('hide').removeClass('modalloading');
                location.reload(true);
    ...
    $(document).on('click', '.adding_printInv', function() {
        ...
        $.ajax({
            ...
            data: {
                ...
                'fee_category': adding_fee_category,
                'fee_groups_feetype_id': adding_fee_groups_feetype_id,
                'fee_master_id': adding_fee_master_id,
                'fee_session_group_id': adding_fee_session_group_id,
                'student_session_id': adding_student_session_id
            },
            beforeSend: function() {
                $this.button('loading');
    ...
    $(document).ready(function() {
        ...
        function fetchAccountTypes(selectedValue) {
            $.ajax({
                ...
                data: {
                    'accountcategory_id': selectedValue
                },
                dataType: "json",
                success: function(data) {
                    var options = '';
...
<script>
    ...
    $(document).ready(function() {
        ...
        // Sidebar Toggle Logic with Scroll Lock & DOM Move
        // Move sidebar to body to ensure it sits above overlay
        var $sidebar = $('.studentsidebar');
        if ($sidebar.length > 0 && $sidebar.parent()[0].tagName !== 'BODY') {
            $sidebar.appendTo('body');
        }

        $(document).on('click', '.studentsideopen', function() {
        ...
        $(document).on('click', '.studentsideclose, .sidebar-overlay', function() {
    ...
    $(document).ready(function() {
        console.log('Initializing Modern Sidebar Logic');

        // Move sidebar to body
        var $sidebar = $('.modern-sidebar-container');
        if ($sidebar.length > 0 && $sidebar.parent()[0].tagName !== 'BODY') {
            $sidebar.appendTo('body');
        }

        $(document).on('click', '.modern-sidebar-toggle', function() {
            console.log('Sidebar toggle clicked');
            $('.modern-sidebar-container').addClass('active');
            $('body').addClass('sidebar-open-active');

            // Add overlay if not exists
            if ($('.modern-sidebar-overlay').length === 0) {
                $('body').append('<div class="modern-sidebar-overlay"></div>');
            }
            setTimeout(function() {
                $('.modern-sidebar-overlay').addClass('active');
            }, 10);
        });

        $(document).on('click', '.modern-sidebar-close, .modern-sidebar-overlay', function() {
            $('.modern-sidebar-container').removeClass('active');
            $('body').removeClass('sidebar-open-active');
            $('.modern-sidebar-overlay').removeClass('active');
