<style type="text/css">
    ...
    /* Style for hours display in attendance cells */
    .tdcls .detail_popover small {
        display: block;
        font-size: 10px;
        font-weight: bold;
    ...
    /* Ensure hours are visible in print and export */
    @media print {
        .tdcls .detail_popover small {
...
$att_type = str_replace(" ", "_", strtolower($value_type['type']));
    ...
    ?>
                            ...
                            <div class="box-body table-responsive table-has-scroll">
                                <?php
if (!empty($resultlist)) {
        ?>
        ...
        <?php echo $this->lang->line('staff_date'); ?>
                                                ...
                                                <th><br/><span data-toggle="tooltip" title="Total Hours Worked">Hours</span></th>
                                                <?php
...
echo date('d', $this->customlib->dateyyyymmddTodateformat($at_value)) . "<br/>" .
        ...
        ?>
            ...
            foreach ($student_array as $student_key => $student_value) {
                $present = isset($monthAttendance[$i][$student_value['id']]['present']) ? $monthAttendance[$i][$student_value['id']]['present'] : 0;
                $late = isset($monthAttendance[$i][$student_value['id']]['late']) ? $monthAttendance[$i][$student_value['id']]['late'] : 0;
                $half_day = isset($monthAttendance[$i][$student_value['id']]['half_day']) ? $monthAttendance[$i][$student_value['id']]['half_day'] : 0;
                $absent = isset($monthAttendance[$i][$student_value['id']]['absent']) ? $monthAttendance[$i][$student_value['id']]['absent'] : 0;
                $holiday = isset($monthAttendance[$i][$student_value['id']]['holiday']) ? $monthAttendance[$i][$student_value['id']]['holiday'] : 0;
                
                $total_present = ($present + $late + $half_day);
                $total_days = $present + $late + $absent + $half_day;
                ...
                ?>
                                                    <tr>
                                                        ...
                                                        <th><strong style="color: #00a65a;"><?php echo isset($monthlyHours[$student_value['id']]) ? $monthlyHours[$student_value['id']] : '0:00 hrs'; ?></strong></th>
                                                        <th><?php echo $present; ?></th>
                                                        <th><?php echo $late; ?></th>
                                                        <th><?php echo $absent; ?></th>
                                                        <th><?php echo $half_day; ?></th>
                                                        <th><?php echo $holiday; ?></th>
                                                        <?php
foreach ($attendence_array as $at_key => $at_value) {
                    ?>
                                                            <th class="tdcls text text-center">
                                                                <span data-toggle="popover" class="detail_popover" data-original-title="" title="">
                                                                    <a href="#" style="color:#333">
                                                                        <?php
if (isset($resultlist[$at_value][$student_value['id']]['key'])) {
    echo $resultlist[$at_value][$student_value['id']]['key'];
    
    // Display hours alongside the attendance key
    if (isset($resultlist[$at_value][$student_value['id']]['hours_worked']) && 
        $resultlist[$at_value][$student_value['id']]['hours_worked'] != '-') {
        echo '<br><small style="color: #00a65a; font-weight: bold;">(' . 
             $resultlist[$at_value][$student_value['id']]['hours_worked'] . ')</small>';
    }
} else {
    echo '-';
}
                    ?>
                                                                ...
                                                                <div class="fee_detail_popover" style="display: none">
                                                                    <?php
if (isset($resultlist[$at_value][$student_value['id']]['remark']) && !empty($resultlist[$at_value][$student_value['id']]['remark'])) {
                        echo '<strong>Remark:</strong> ' . $resultlist[$at_value][$student_value['id']]['remark'] . '<br>';
                    }
                    
                    // Display hours worked
                    if (isset($resultlist[$at_value][$student_value['id']]['hours_worked']) && $resultlist[$at_value][$student_value['id']]['hours_worked'] != '-') {
                        echo '<strong style="color: #00a65a;">Hours Worked: ' . $resultlist[$at_value][$student_value['id']]['hours_worked'] . '</strong><br>';
                    }
                    
                    // Display check-in time
                    if (isset($resultlist[$at_value][$student_value['id']]['first_checkin_time']) && !empty($resultlist[$at_value][$student_value['id']]['first_checkin_time'])) {
                        echo '<strong>First Check-in:</strong> ' . $resultlist[$at_value][$student_value['id']]['first_checkin_time'] . '<br>';
                    }
                    
                    // Display check-out time
                    if (isset($resultlist[$at_value][$student_value['id']]['last_checkout_time']) && !empty($resultlist[$at_value][$student_value['id']]['last_checkout_time'])) {
                        echo '<strong>Last Check-out:</strong> ' . $resultlist[$at_value][$student_value['id']]['last_checkout_time'];
                    }
                    ?>
                                                                </div>
                                                            </th>
