                            ...
                            foreach ($side_list as $side_list_key => $side_list_value) {
                                ...
                                if (!empty($module_permission)) {
                                    foreach ($module_permission as $m_permission_key => $m_permission_value) {
                                        ...
                                        if (isset($cat_permission[0]) && isset($cat_permission[1])) {
                                                        ...
                                                        if (!empty($sidebar_permission)) {
                                                            foreach ($sidebar_permission as $sidebar_permission_key => $sidebar_permission_value) {
                                                                ...
                                                                if ($submenu_value->addon_permission != "") {
                                                                    ...
                                                                    if (isset($sidebar_cat_permission[0]) && isset($sidebar_cat_permission[1])) {
...
?>";
    ...
    $calendar.fullCalendar({
        viewRender: function (view, element) {
            // Update Custom Header Title automatically
            $('#cal-title').text(view.title);
            
            // Sync active state of our custom view buttons
            $('.cal-btn-view').removeClass('active');
            $('.cal-btn-view[data-view="' + view.name + '"]').addClass('active');
        },

        header: false, // Disable default header to use our custom integrated one
        firstDay: start_week,
        defaultDate: today,
        defaultView: viewtitle,
        selectable: true,
        selectHelper: true,
        nowIndicator: true, // Shows a line at the current time
        views: {
            month: {// name of view
...
<?php
    ...
    // Custom Calendar Header Bindings
    $(document).on('click', '.cal-prev-btn', function() { $calendar.fullCalendar('prev'); });
    $(document).on('click', '.cal-next-btn', function() { $calendar.fullCalendar('next'); });
    $(document).on('click', '.cal-btn-today', function() { $calendar.fullCalendar('today'); });
    
    $(document).on('click', '.cal-btn-view', function() {
        var view = $(this).data('view');
        $calendar.fullCalendar('changeView', view);
    });

    $(document).on('click', '.cal-btn-new-event', function() {
        var newEventModal = $('#newEventModal');
        $("#input-field").val('');
        $("#desc-field").text('');
 ...
 $('.event_to',__viewModal).data("DateTimePicker").date(event_end_to);
    ...
    /* Live update for the 'Now' indicator tooltip */
    function updateNowIndicatorTime() {
        var now = new Date();
        var timeStr = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        $('.fc-now-indicator-line').attr('data-time', 'CURRENT TIME: ' + timeStr);
    }
    
    // Update every 10 seconds to keep it fresh
    setInterval(updateNowIndicatorTime, 10000);
    // Also run immediately
    setTimeout(updateNowIndicatorTime, 1000);
