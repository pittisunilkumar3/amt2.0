...
<script type="text/javascript" src="<?php echo base_url(); ?>backend/plugins/datepicker/bootstrap-datepicker.js"></script>

<style>
    /* Scoped Container to prevent leakage to Sidebar/Header */
    #columnwise-report-page .summary-container {
        display: grid;
    ...
    #columnwise-report-page .summary-card {
        background: #fff;
        border-radius: 12px;
        overflow: hidden;
        display: flex;
    ...
    #columnwise-report-page .summary-card:hover {
        transform: translateY(-4px);
        box-shadow: 0 8px 25px rgba(0,0,0,0.1);
    }

    #columnwise-report-page .summary-card .icon-box {
        width: 55px;
        display: flex;
        align-items: center;
    ...
    #columnwise-report-page .summary-card .content-box {
        padding: 12px 18px;
        flex-grow: 1;
        display: flex;
    ...
    #columnwise-report-page .summary-card .label {
        font-size: 11px;
    ...
    #columnwise-report-page .summary-card .value {
        font-size: 19px;
        font-weight: 800;
        color: #2c3e50;
    }

    #columnwise-report-page .bg-gradient-blue { background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); }
    #columnwise-report-page .bg-gradient-green { background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); }
    #columnwise-report-page .bg-gradient-orange { background: linear-gradient(135deg, #f12711 0%, #f5af19 100%); }
    #columnwise-report-page .bg-gradient-red { background: linear-gradient(135deg, #eb3349 0%, #f45c43 100%); }

    /* Dynamic table styling with horizontal scroll */
    #columnwise-report-page .table-responsive-scroll {
    ...
    #columnwise-report-page #headerTable {
    ...
    #columnwise-report-page #headerTable th {
    ...
    #columnwise-report-page #headerTable td {
        border: 1px solid #ddd !important;
    ...
    #columnwise-report-page #headerTable thead tr:first-child th {
    ...
    #columnwise-report-page #headerTable thead tr:nth-child(2) th {
    ...
    #columnwise-report-page #headerTable .total-bg {
        background-color: #2c3e50 !important;
        color: white !important;
        font-weight: bold !important;
    }

    #columnwise-report-page .fee-column {
        min-width: 100px !important;
    }

    #columnwise-report-page .student-info {
        min-width: 120px !important;
    }

    /* Highlight total columns at the end */
    #columnwise-report-page #headerTable td:nth-last-child(-n+3) {
        background-color: #fff9f0 !important;
        font-weight: 700 !important;
    }

    #columnwise-report-page #headerTable td:last-child {
        background-color: #fef5f5 !important;
        color: #c0392b !important;
        font-weight: 900 !important;
    }

    /* SumoSelect Enhancements (Scoped) */
    #columnwise-report-page .SumoSelect { width: 100% !important; }
    #columnwise-report-page .SumoSelect > .CaptionCont {
        border: 1px solid #d2d6de;
        border-radius: 3px;
        min-height: 34px;
    }

    #columnwise-report-page .form-group label {
        font-weight: 600;
        color: #444;
        margin-bottom: 5px;
        display: block;
    ...
    @media print {
        ...
        #columnwise-report-page .summary-container, 
        #columnwise-report-page .search_btn, 
        #columnwise-report-page .box-header,
        #columnwise-report-page .btn { display: none !important; }
        #columnwise-report-page .table-responsive-scroll { overflow: visible !important; }
    }
</style>
<div class="content-wrapper" id="columnwise-report-page">
    ...
    ?>
                    <div class="">
                        ...
                        <div class="box-body" id="transfee">
                            ...
                            <?php
                            // Calculate totals BEFORE rendering anything to show in summary cards
                            $summary_total = 0;
                            $summary_paid = 0;
                            $summary_balance = 0;
                            $processed_students = array();
                            $temp_dynamic_fee_types = array();

                            if (isset($fee_types) && is_array($fee_types)) {
                                foreach ($fee_types as $ft) {
                                    $temp_dynamic_fee_types[$ft['type']] = true;
                                }
                            }

                            if (!empty($results)) {
                                foreach ($results as $student) {
                                    $s_key = $student['admission_no'];
                                    if (!isset($processed_students[$s_key])) {
                                        $processed_students[$s_key] = array('total' => 0, 'paid' => 0, 'balance' => 0);
                                    }
                                    
                                    if (isset($student['fee_types']) && is_array($student['fee_types'])) {
                                        foreach ($student['fee_types'] as $type => $fee_data) {
                                            if (is_numeric($fee_data)) {
                                                $processed_students[$s_key]['paid'] += $fee_data;
                                                $processed_students[$s_key]['total'] += $fee_data;
                                            } else {
                                                $processed_students[$s_key]['paid'] += isset($fee_data['paid_amount']) ? $fee_data['paid_amount'] : 0;
                                                $processed_students[$s_key]['total'] += isset($fee_data['total_amount']) ? $fee_data['total_amount'] : 0;
                                                $processed_students[$s_key]['balance'] += isset($fee_data['remaining_amount']) ? $fee_data['remaining_amount'] : 0;
                                            }
                                        }
                                    }
                                }
                                foreach ($processed_students as $ps) {
                                    $summary_total += $ps['total'];
                                    $summary_paid += $ps['paid'];
                                    $summary_balance += $ps['balance'];
                                }
                            }
                            ?>
                            ...
                            <div class="summary-container">
                                <div class="summary-card">
                                    ...
                                    <div class="content-box">
                                        ...
                                        <span class="value"><?php echo $currency_symbol . number_format($summary_total, 2); ?></span>
                                    </div>
                                </div>
                                <div class="summary-card">
                                    ...
                                    <div class="content-box">
                                        ...
                                        <span class="value"><?php echo $currency_symbol . number_format($summary_paid, 2); ?></span>
                                    </div>
                                </div>
                                <div class="summary-card">
                                    ...
                                    <div class="content-box">
                                        ...
                                        <span class="value"><?php echo $currency_symbol . number_format($summary_balance, 2); ?></span>
                                    </div>
                                </div>
                            </div>
                        ...
                        </div> <!-- End table-responsive-scroll -->
                    </div> <!-- End box-body transfee -->
                </div> <!-- End div from line 692 -->
            </div> <!-- End col-md-12 -->
        </div> <!-- End row -->
                                        <?php
}
