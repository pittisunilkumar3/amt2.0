...
class Tcgeneration_model extends MY_Model
{
    ...
    public function add_tc_report($data)
    {
        $this->db->insert('student_tc_reports', $data);
        return $this->db->insert_id();
    }

    public function get_tc_report($class_id = null, $section_id = null)
    {
        $this->db->select('student_tc_reports.*, students.firstname, students.middlename, students.lastname, students.admission_no, classes.class, sections.section, tc_generation.tc_head_tittle as template_name, staff.name as staff_firstname, staff.surname as staff_lastname')->from('student_tc_reports');
        $this->db->join('students', 'students.id = student_tc_reports.student_id');
        $this->db->join('student_session', 'student_session.student_id = students.id AND student_session.session_id = student_tc_reports.session_id');
        $this->db->join('classes', 'classes.id = student_session.class_id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('tc_generation', 'tc_generation.id = student_tc_reports.certificate_id');
        $this->db->join('staff', 'staff.id = student_tc_reports.generated_by');
        ...
        $this->db->order_by('student_tc_reports.generated_date', 'desc');
        $query = $this->db->get();
        return $query->result_array();
    }
}
