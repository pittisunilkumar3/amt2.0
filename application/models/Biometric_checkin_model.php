...
class Biometric_checkin_model extends CI_Model
{
    ...
    public function getStaffCheckinSummary($date = null)
    {
        ...
        foreach ($all_staff as $staff) {
            ...
            // Calculate total hours worked (from first check-in to last check-out)
            $staff_data['total_hours'] = '-';
            $staff_data['total_hours_decimal'] = 0;
            if ($staff_data['has_checked_in'] && $staff_data['has_checked_out']) {
                $first_checkin_timestamp = strtotime($checkin_records[0]['created_at']);
                $last_checkout_timestamp = strtotime(end($checkout_records)['created_at']);
                
                $duration_seconds = $last_checkout_timestamp - $first_checkin_timestamp;
                $hours = floor($duration_seconds / 3600);
                $minutes = floor(($duration_seconds % 3600) / 60);
                
                $staff_data['total_hours'] = sprintf('%d:%02d hrs', $hours, $minutes);
                $staff_data['total_hours_decimal'] = round($duration_seconds / 3600, 2);
            }
