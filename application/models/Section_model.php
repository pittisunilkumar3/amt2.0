...
class Section_model extends MY_Model {
    ...
    public function getSectionsByClassIds($class_ids) {
        if (empty($class_ids)) {
            return array();
        }
        ...
        $this->db->select('class_sections.section_id, sections.section');
        $this->db->from('class_sections');
        $this->db->join('sections', 'sections.id = class_sections.section_id');
        $this->db->where_in('class_sections.class_id', $class_ids);
        
        // Apply teacher restriction if applicable
        if (isset($role_id) && ($userdata["role_id"] == 2) && ($userdata["class_teacher"] == "yes")) {
            $staff_id = $userdata["id"];
            
            // We need sections where teacher is assigned to subject OR is class teacher for that section
            $this->db->group_start();
            $this->db->where("class_sections.section_id IN (SELECT st.section_id FROM subject_timetable st WHERE st.staff_id = $staff_id AND st.session_id = $current_session AND st.class_id IN (".implode(',', $class_ids)."))");
            $this->db->or_where("class_sections.section_id IN (SELECT ct.section_id FROM class_teacher ct WHERE ct.staff_id = $staff_id AND ct.session_id = $current_session AND ct.class_id IN (".implode(',', $class_ids)."))");
            $this->db->group_end();
        }
