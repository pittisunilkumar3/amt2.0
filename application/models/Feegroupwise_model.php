...
class Feegroupwise_model extends CI_Model
{
    ...
    {
        // First, get all fee groups with their total assigned amounts
        // We join student_fees_master to get assigned students and fee_groups_feetype to get the amounts
        $sql = "
            SELECT
                fg.id as fee_group_id,
                fg.name as fee_group_name,
                COALESCE(SUM(fgft.amount), 0) as total_amount,
    ...
    {
        // First, get all fee groups with their total assigned amounts
        // We join student_fees_masteradding to get assigned students and fee_groups_feetypeadding to get the amounts
        $sql = "
            SELECT
                fga.id as fee_group_id,
                fga.name as fee_group_name,
                COALESCE(SUM(fgfta.amount), 0) as total_amount,
            ...
            INNER JOIN fee_groups_feetypeadding fgfta ON fgfta.fee_session_group_id = fsga.id
    ...
    private function calculateCollectedAmount($fee_group_id, $session_id, $class_ids, $section_ids, $from_date, $to_date, $type = 'regular')
    {
        ...
        if ($type == 'regular') {
            ...
            $where_clause = " WHERE fgft.fee_groups_id = ?";
            ...
            if (!empty($class_ids) || !empty($section_ids)) {
                ...
                if (!empty($class_ids)) {
                    $placeholders = implode(',', array_fill(0, count($class_ids), '?'));
                    $where_clause .= " AND ss.class_id IN ($placeholders)";
                    $params = array_merge($params, $class_ids);
                }

                if (!empty($section_ids)) {
                    $placeholders = implode(',', array_fill(0, count($section_ids), '?'));
                    $where_clause .= " AND ss.section_id IN ($placeholders)";
                    $params = array_merge($params, $section_ids);
                }
            }
            ...
            $where_clause = " WHERE fgfta.fee_groups_id = ?";
            $params = array($fee_group_id);
            ...
            if (!empty($class_ids) || !empty($section_ids)) {
                ...
                if (!empty($class_ids)) {
                    $placeholders = implode(',', array_fill(0, count($class_ids), '?'));
                    $where_clause .= " AND ssa.class_id IN ($placeholders)";
                    $params = array_merge($params, $class_ids);
                }

                if (!empty($section_ids)) {
                    $placeholders = implode(',', array_fill(0, count($section_ids), '?'));
                    $where_clause .= " AND ssa.section_id IN ($placeholders)";
                    $params = array_merge($params, $section_ids);
                }
            }
    ...
    private function getRegularFeesDetailedData($session_id, $class_ids, $section_ids, $feegroup_ids, $from_date, $to_date)
    {
        ...
        $sql = "
            SELECT
                ...
                fg.name as fee_group_name,
                (SELECT SUM(amount) FROM fee_groups_feetype WHERE fee_session_group_id = fsg.id) as total_amount,
    ...
    private function getAdditionalFeesDetailedData($session_id, $class_ids, $section_ids, $feegroup_ids, $from_date, $to_date)
    {
        ...
        $sql = "
            SELECT
                ...
                sec.section as section_name,
                fga.name as fee_group_name,
                (SELECT SUM(amount) FROM fee_groups_feetypeadding WHERE fee_session_group_id = fsga.id) as total_amount,
    ...
    /**
     * Get collection trend data for charts
     */
    public function getCollectionTrend($session_id, $class_ids, $section_ids, $feegroup_ids, $from_date, $to_date, $grouping = 'none')
    {
        $trend = array();
        ...
        // We'll reuse the logic from calculateCollectedAmount but aggregate by date
        $sql = "
            SELECT sfd.amount_detail, sfd.fee_groups_feetype_id
            FROM student_fees_deposite sfd
            INNER JOIN student_fees_master sfm ON sfm.id = sfd.student_fees_master_id
            INNER JOIN fee_groups_feetype fgft ON fgft.id = sfd.fee_groups_feetype_id
            INNER JOIN fee_session_groups fsg ON fsg.id = fgft.fee_session_group_id AND fsg.session_id = ?
        ";
        ...
        if (!empty($feegroup_ids)) {
            $placeholders = implode(',', array_fill(0, count($feegroup_ids), '?'));
            $where .= " AND fgft.fee_groups_id IN ($placeholders)";
            $params = array_merge($params, $feegroup_ids);
        }
        ...
        if (!empty($class_ids) || !empty($section_ids)) {
            ...
            if (!empty($class_ids)) {
                $placeholders = implode(',', array_fill(0, count($class_ids), '?'));
                $where .= " AND ss.class_id IN ($placeholders)";
                $params = array_merge($params, $class_ids);
            }
            if (!empty($section_ids)) {
                $placeholders = implode(',', array_fill(0, count($section_ids), '?'));
                $where .= " AND ss.section_id IN ($placeholders)";
                $params = array_merge($params, $section_ids);
            }
        }
        ...
        foreach ($results as $row) {
            if (!empty($row->amount_detail)) {
                $details = json_decode($row->amount_detail);
                if (is_array($details) || is_object($details)) {
                    foreach ($details as $detail) {
                        if (isset($detail->date) && isset($detail->amount)) {
                            $date = date('Y-m-d', strtotime($detail->date));
                            if ($date >= $from_date && $date <= $to_date) {
                                if (!isset($daily_data[$date])) $daily_data[$date] = 0;
                                $daily_data[$date] += floatval($detail->amount);
                            }
                        }
                    }
                }
        ...
        // Group the daily data if requested
        if ($grouping == 'daily' || $grouping == 'none') {
            foreach ($daily_data as $date => $amount) {
                $trend[] = array('label' => date('d M', strtotime($date)), 'amount' => $amount);
            }
            ...
            foreach ($weekly as $week => $amount) {
                $trend[] = array('label' => 'Week ' . substr($week, 5), 'amount' => $amount);
            }
            ...
            foreach ($monthly as $month => $amount) {
                $trend[] = array('label' => date('M Y', strtotime($month . '-01')), 'amount' => $amount);
            }
        }
    ...
    public function getAllFeeGroups($session_id = null, $class_ids = array(), $section_ids = array())
    {
        if ($session_id === null) {
            $session_id = $this->current_session;
        }

        $this->db->select('DISTINCT(fg.id), fg.name');
        $this->db->from('fee_groups fg');
        $this->db->join('fee_session_groups fsg', 'fsg.fee_groups_id = fg.id');
        
        if (!empty($class_ids) || !empty($section_ids)) {
            // Filter fee groups that are actually assigned to students in these classes/sections
            $this->db->join('student_fees_master sfm', 'sfm.fee_session_group_id = fsg.id');
            $this->db->join('student_session ss', 'ss.id = sfm.student_session_id');
            
            if (!empty($class_ids)) {
                $this->db->where_in('ss.class_id', $class_ids);
            }
            if (!empty($section_ids)) {
