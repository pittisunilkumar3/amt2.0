...
class Student_model extends MY_Model
{
    ...
    public function getSearchFullView($searchterm, $start, $limit, $search, $carray)
    {
      ...
      $field_variable = (empty($field_var_array))? "": ",".implode(',', $field_var_array);
        ...
        $this->db->select('classes.id AS `class_id`,students.id,student_session.id as student_session_id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,students.middlename,  students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode ,     students.religion,     students.dob ,students.current_address,    students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,      students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code ,students.father_name , students.guardian_name , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.gender,students.rte,student_session.session_id,sessions.session as session_name,' . $field_variable)->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        ...
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
        $this->db->join('school_houses', 'students.school_house_id = school_houses.id', 'left');
        // Removed session filter to search across ALL sessions
        // $this->db->where('student_session.session_id', $this->current_session);
        $this->db->where('students.is_active', 'yes');
        $this->db->group_start();
        $this->db->like('students.firstname', $searchterm);
    ...
    public function searchFullText($searchterm, $carray = null)
    {
        ...
        $this->datatables->select('classes.id AS `class_id`,students.id,student_session.id as student_session_id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,students.middlename,students.lastname,students.image,  students.mobileno, students.email ,students.state,students.city,students.pincode,students.religion,     DATE(students.dob) as dob ,students.current_address,students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,      students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code ,students.father_name,students.guardian_name, students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.gender,students.rte,student_session.session_id,sessions.session as session_name' . $field_variable);
        $this->datatables->join('student_session', 'student_session.student_id = students.id');
        $this->datatables->join('classes', 'student_session.class_id = classes.id');
        $this->datatables->join('sections', 'sections.id = student_session.section_id');
        $this->datatables->join('sessions', 'student_session.session_id = sessions.id');
        $this->datatables->join('categories', 'students.category_id = categories.id', 'left');
        $this->datatables->join('school_houses', 'students.school_house_id = school_houses.id', 'left');
        ...
        // $this->datatables->where('student_session.session_id', $this->current_session);
        $this->datatables->where('students.is_active', 'yes');
        $this->datatables->searchable('students.admission_no,students.firstname,students.middlename,students.lastname,classes.class,students.father_name,students.dob,students.gender,categories.category,students.mobileno,sessions.session' . $field_variable);
        $this->datatables->orderable('students.admission_no,students.firstname,classes.class,students.father_name,dob,students.gender,categories.category,students.mobileno,sessions.session' . $field_name);
        $this->datatables->sort('student_session.session_id', 'desc');
        $this->datatables->from('students');
        $std_data = $this->datatables->generate('json');
    ...
    public function updatereft($data) {
        $this->db->where('student_id', $data['student_id']);
        $query = $this->db->get('student_reference');
        
        if ($query->num_rows() > 0) {
            // Update existing record
            $this->db->where('student_id', $data['student_id']);
            $result = $this->db->update('student_reference', $data);
        } else {
            // Insert new record
            $result = $this->db->insert('student_reference', $data);
        }
    ...
    public function update_student_session($data, $session)
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            // Do not insert new record, just return false
            $this->db->trans_complete();
            return false;
        }
     ...
     public function searchByClassSectionAnddiscountStatus($class_id,$certifid,$section_id = null,$statuss=null,$session_id=null)
    {
        ...
        $field_variable = implode(',', $field_var_array);
        $this->db->select('fees_discount_approval.payment_id,fees_discount_approval.id as fdaid,fee_groups.name as fgrname,fees_discount_approval.amount,fees_discount_approval.description as discount_note,fees_discount_approval.student_session_id AS stid,fees_discount_approval.approval_status,classes.id AS `class_id`,student_session.id as student_session_id,students.id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,students.middlename,students.lastname,students.image,  students.mobileno,students.email,students.state,students.city,students.pincode,students.religion,students.dob ,students.current_address,students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code, students.guardian_name, students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.app_key,students.parent_app_key,students.rte,students.gender,vehicles.vehicle_no,transport_route.route_title,route_pickup_point.id as `route_pickup_point_id`,pickup_point.name as `pickup_point`,' . $field_variable)->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
        ...
        $this->db->join('fees_discount_approval','fees_discount_approval.student_session_id=student_session.id');
        // $this->db->where('fees_discount_approval.fees_discount_id',$certifid);
        //$this->db->join('fees_discount_approval','fees_discount_approval.student_session_id=students.id','left');
        $this->db->join('student_fees_master','fees_discount_approval.student_fees_master_id=student_fees_master.id');
