...
class Mailsmsconf
{
    ...
    public function mailsms($send_for, $sender_details, $date = null, $exam_schedule_array = null, $file = null)
    {
        ...
        if (isset($student_session_id) && !empty($student_session_id)) {
            ...
            // Collect ALL additional phone numbers (father, mother) for WhatsApp & SMS
            $all_phones = array();
            if (!empty($recipient_data['mobileno'])) {
                $all_phones[] = $recipient_data['mobileno'];
            }
            if (!empty($recipient_data['guardian_phone'])) {
                $all_phones[] = $recipient_data['guardian_phone'];
            }
            if (!empty($recipient_data['father_phone'])) {
                $all_phones[] = $recipient_data['father_phone'];
            }
            if (!empty($recipient_data['mother_phone'])) {
                $all_phones[] = $recipient_data['mother_phone'];
            }
            $contact_numbers = array_values(array_unique(array_filter($all_phones)));
        }
        ...
        if (!empty($chk_mail_sms)) {
            if ($send_for == "student_admission") {
                ...
                // ---- WhatsApp ----
                if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" && !empty($chk_mail_sms['whatsapp_template_id'])) {
            ...
            if ($chk_mail_sms['sms'] && $chk_mail_sms['template'] != "" && !empty($sms_detail)) {
                ...
                if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" && !empty($chk_mail_sms['whatsapp_template_id'])) {
                    ...
                    if (!empty($contact_numbers)) {
                        foreach ($contact_numbers as $key => $contact_numbersvalue) {
                            $this->CI->whatsappgateway->sentFeeProcessingNotification($sender_details, $contact_numbersvalue, $chk_mail_sms['template'], $chk_mail_sms['whatsapp_template_id']);
                        }
                    }
                }
                ...
                if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" && !empty($chk_mail_sms['whatsapp_template_id'])) {
                    $this->CI->load->library('whatsappgateway');
                    foreach ($contact_numbers as $key => $contact_numbersvalue) {
                        $this->CI->whatsappgateway->sentfeesreminderNotification($sender_details, $contact_numbersvalue, $chk_mail_sms['template'], $chk_mail_sms['whatsapp_template_id']);
                    }
                }
                ...
                $this->sendOnlineexam($chk_mail_sms, $sender_details, $chk_mail_sms['template'], $chk_mail_sms['subject'], $chk_mail_sms['template_id'],$chk_mail_sms['whatsapp_template_id']);
                ...
                if ($chk_mail_sms['mail'] && $chk_mail_sms['template'] != "" && $chk_mail_sms['staff_recipient']) {
                    ...
                    $this->sendOnlineadmission($chk_mail_sms, $sender_details, $chk_mail_sms['template'], $chk_mail_sms['subject'], $chk_mail_sms['template_id'],$chk_mail_sms['whatsapp_template_id']);
                    ...
                    $this->sendOnlineadmissionFees($chk_mail_sms, $sender_details, $chk_mail_sms['template'], $chk_mail_sms['subject'], $chk_mail_sms['template_id'],$chk_mail_sms['whatsapp_template_id']);
                ...
                // ---- WhatsApp (guardian) ----
                if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" && !empty($chk_mail_sms['whatsapp_template_id']) && $chk_mail_sms['guardian_recipient']) {
                    $this->CI->load->library('whatsappgateway');
                    if (!empty($contact_numbers)) {
                        foreach ($contact_numbers as $key => $contactvalue) {
                            $sender_details['contact_no'] = $contactvalue;
    ...
    public function sendOnlineexam($chk_mail_sms, $student_details, $template, $subject, $template_id,$whatsapp_template_id)
    {
        $student_sms_list          = array();
        $student_email_list        = array();
        $student_notification_list = array();
    ...
    public function sendOnlineadmission($chk_mail_sms, $student_details, $template, $subject, $template_id,$whatsapp_template_id)
    {
    ...
    public function sendOnlineadmissionFees($chk_mail_sms, $student_details, $template, $subject, $template_id,$whatsapp_template_id)
    {
        $student_sms_list          = array();
        $student_email_list        = array();
        $student_notification_list = array();
