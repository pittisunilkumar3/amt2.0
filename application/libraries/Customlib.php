...
class Customlib
{
    ...
    public function getBaseUrl()
    {
        ...
        if (isset($admin['db_array']['base_url'])) {
    ...
    {
        $admin       = $this->CI->session->userdata('admin');
        if (isset($admin['db_array']['folder_path'])) {
        ...
        if (!empty($notifications)) {
            foreach ($notifications as $note_key => $note_value) {
                if ($note_value->type == $find) {
                    return array(
                        'whatsapp'               => (isset($note_value->is_whatsapp)) ? $note_value->is_whatsapp : 0,
                        'whatsapp_template_id'  => (isset($note_value->whatsapp_template_id)) ? $note_value->whatsapp_template_id : '',
    ...
    public function getFullNamee($firstname, $middlename, $lastname, $is_middlename, $is_lastname)
    {
        $name = "";
        if ($is_middlename) {
            $name = ($middlename == "") ? $firstname : $firstname . " " . $middlename;
        ...
        if ($is_lastname) {
            $name = ($lastname == "") ? $name : $name . " " . ucfirst($lastname[0]);
        }
     ...
     public function checkadminstatus(){
        $progress           = array();
        $progress['noadmissionno']   = $this->CI->lang->line('noadmissionno');
        $progress['withadmissionno'] = $this->CI->lang->line('withadmissionno');
        return $progress;
    }

    public function resultaddingstatus(){
        $progress           = array();
        $progress['noaddresult']   = $this->CI->lang->line('noadmissionno');
        $progress['addresult'] = $this->CI->lang->line('withadmissionno');
        return $progress;
    }
    
    public function passstatus(){
        $progress = array();
        $progress['fail'] = $this->CI->lang->line('fail');
        $progress['pass'] = $this->CI->lang->line('pass');
    ...
    {
        $gender           = array();
        $gender['creditor']   = $this->CI->lang->line('creditor');
        $gender['debitor'] = $this->CI->lang->line('debitor');
        $gender['both'] = $this->CI->lang->line('both');
        return $gender;
    }
     ...
     */
    public function getOrdinalSuffix($num)
    {
    ...
    /**
     * Convert working_days_method database value to readable label
     */
    public function humanizeWorkingDaysMethod($method)
    {
        $map = array(
            'exclude_sundays'         => 'Mon–Sat (No Sundays)',
            'exclude_sundays_saturdays' => 'Mon–Fri (No Weekends)',
            'include_all'             => 'All Days',
            'fixed'                   => 'Fixed Days',
        );
        return isset($map[$method]) ? $map[$method] : ucfirst(str_replace('_', ' ', $method));
    }
