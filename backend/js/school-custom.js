...
/* Global Form Submission Guard for SumoSelect persistence
   Prevents unintended form resets from SumoSelect "OK" button or accidental triggers.
*/
$(document).on('click', 'button[type=submit], input[type=submit]', function () {
    $(this).closest('form').find('button[type=submit], input[type=submit]').removeAttr('clicked');
...
$(document).on('submit', 'form', function (e) {
    ...
    // If the form has submit buttons, ensure one was explicitly focused or clicked
    if ($submitBtns.length > 0) {
        var $clickedBtn = $form.find('button[type=submit][clicked=true], input[type=submit][clicked=true]');
        if (!$clickedBtn.length) {
            var activeElement = document.activeElement;
            var isSubmitFocused = $(activeElement).is('button[type=submit], input[type=submit]');

            if (!isSubmitFocused) {
                // Check if this form explicitly wants to skip the guard
                if ($form.data('skip-guard')) return;

                // Block unintended submission (likely from a secondary button or SumoSelect)
                console.log('🛡️ Global Guard: Unintended form submission blocked.');
                e.preventDefault();
                return false;
            }
        }
